 ...
 * <pre>
   HTTP请求代理类
 * 连接超时/读取数据超时/请求编码可以通过config.properties文件进行设置.
 * </pre>
 * @author xiangshangwen
...
/*	static
	{
		try
		{
			HttpRequestProxy.connectTimeOut = Integer.parseInt(PropertiesUtil.getProperty((
					HTTP_REQUEST_PROXY_CONNECT_TIME_OUT).toString()));
			HttpRequestProxy.readTimeOut = Integer.parseInt(PropertiesUtil.getProperty(HTTP_REQUEST_PROXY_READ_TIME_OUT)
					.toString());
		}
		catch (Exception e)
		{
